...
var firmidlistarray = ['allenco','bankofamerica','barclays','baird','bmo','centerview','citigroup','creditsuisse','deutschebank','ducerapartners','evercore','financo','goldmansachs','gordandyal','greenhill','guggenheim','houlihanlokey','jefferies','jpmorgan','lazard','lincoln','macquarie','mizuho','mkleinco','moelis','morganstanley','perellaweinberg','pjt','raine','rbc','rothschildco','stifel','stout','solomonpartners','tdcowen','ubs','wellsfargo','williamblair']
...
// LOGOS
const rothschild_button = document.querySelector('#rothschildco')
rothschild_button.onclick = () =>{
    bankname.innerHTML = "Rothschild & Co"
    writeup.innerHTML = "Rothschild & Co, founded in the early 1800’s, is one of Europe’s oldest and most well known banks. In the US, Rothschild & Co operates as a middle-market firm, with a particularly strong restructuring division. Besides their global advisory services, the bank also has solid wealth management, asset management, and merchant banking arms. The firm takes pride in caring about their employees and the long term health of the company, rather than managing their operations on a quarter to quarter basis. This was reflected by the firms recent decision to go private. Rothschild & Co's values and care for their employees, combined with the firm’s deep European roots, creates a respectable culture that is well known and admired across Wall Street."
    interviewsum.innerHTML = "Rothschild & Co comes to Ann Arbor each year and traditionally accepts 2-4 Michigan students into their analyst class. However, the firm doesn’t place a heavy emphasis on networking calls. It’s important to make a few connections at the firm, but getting an interview is not solely dependent on networking. The first round interview at Rothschild & Cois all technical, testing your understanding and ability to speak concisely about technical content and how they pertain to general business. The first round is followed by a 2 interview super day which is much more behavioral. This allows the upper level bankers to get more of a feel for the candidate."
...
const baird_button = document.querySelector('#baird')
baird_button.onclick = () =>{
    bankname.innerHTML = "Baird"
    writeup.innerHTML = "To be updated"
    interviewsum.innerHTML = "TBD"
...
bofa_button.onclick = () =>{
    ...
    writeup.innerHTML = "Bank of America is a bulge bracket bank, well known for their global advisory services across the United States. After acquiring Merrill Lynch in 2009, Bank of America gained invaluable connections across the business world, and used these connections to become a leader in the investment banking industry. In terms of banking, B of A is known for their strengths in healthcare, TMT, and financial sponsors. With that said, B of A still spans across nearly all industry sectors. Bank of America places a heavy emphasis on recruiting from Michigan, and has an extremely strong Michigan presence within their TMT division."
    interviewsum.innerHTML = "Bank of America is one of the more hands-on recruiting processes in comparison to other banks. Last year, Bank of America set up a table at the winter garden, came to campus for a typical information session, and hosted an invite only dinner in Ann Arbor. Networking definitely holds a strong weight in the BofA recruiting process. These networking opportunities are followed by a first round via HireVue, and then a super day. The superday is standard for a large bulge-bracket firm; including 3 interviews with a mix of behaviorals and technicals."
...
bmo_button.onclick = () =>{
    ...
    interviewsum.innerHTML = "BMO was one of the first banks to recruit at Michigan this past year. They came to campus in late February, and began interviews in late March. Be sure to network early with BMO to give yourself an opportunity to get one of these early interviews. Plus, they have many Michigan alumni, so make sure to leverage that connection. The first round interview is mainly behavioral, with some basic technicals. Likewise, the super day is known to be both behavioral and technical, with the technicals noted to be a little more difficult than the first round."
...
const deutsche_button = document.querySelector('#deutschebank')
deutsche_button.onclick = () =>{
...
const ducera_button = document.querySelector('#ducerapartners')
ducera_button.onclick = () =>{
...
financo_button.onclick = () =>{
    ...
    interviewsum.innerHTML = "Financo’s small analyst class size makes their recruiting process difficult. It is important to network if Financo comes to your campus, and they have recently added UMich to be one of their target schools. The first round interview is typically both behaviorals and basic technicals. The super day, however, is known to be extremely technical, with one of the interviews being 60 minutes straight of technical questions. Another super day interview is more behavioral, with industry specific questions about Financo’s areas of operation. In order to land the job, you need to effectively show why you want to be at Financo specifically."
    emailformat.innerHTML = "firstinitiallastname@financo.com (ie. jsmith@financo.com)"
    banklogo.src = "images/banklogos/financo.png"
}
const ftpartners_button = document.querySelector('#ftpartners')
ftpartners_button.onclick = () =>{
...
goldman_button.onclick = () =>{
    ...
    interviewsum.innerHTML = "The recruiting process at GS is relatively standard of a large bulge-bracket bank. The process takes place after the boutiques, and usually kicks off in May or June. Networking is important here, as they get more applicants than any other firm. First round interviews are done on Hirevue, with questions mainly being behavioral or logic based. Next, the top candidates are selected for super days, which are often more behavioral than technical. In the behavioral section, they want candidates to show their personality outside of academics and finance. In the technical section, they typically ask conceptual, guide-based questions."
    emailformat.innerHTML = "first.last@gs.com (ie. john.smith@gs.com)"
    banklogo.src = "images/banklogos/goldman.png"
}

const gordan_button = document.querySelector('#gordandyal')
gordan_button.onclick = () =>{
...
greenhill_button.onclick = () =>{
    ...
    interviewsum.innerHTML = "Greenhill is known for a smooth, transparent recruiting process. Because the firm is so small, it’s important to get to know the school representatives. Greenhill was one of the first banks to come to Ann Arbor this past spring which is why it is important to start networking as soon as possible. Their analysts are also known for being easier to reach than other firms. The first round interview is followed by a super day of 4 interviews, which are known to be technically challenging. The super day interviews are often conducted by upper level bankers."
...
guggenheim_button.onclick = () =>{
    ...
    writeup.innerHTML = "Guggenheim prides themselves on being a hardworking, driven firm with a collaborative company culture. The firm pushes to “punch above their weight” and are starting to advise on more of the larger deals on the street. Plus, MDs are known to have an entrepreneurial mindset, finding creative ways to solve problems within their strategic advisory services. The firm is also known to have a strong TMT group, which is made up of many Michigan alumni. They offer a generalist program for the internship before entering into an industry or product group full time."
    interviewsum.innerHTML = "Networking is extremely important at Guggenheim. Prioritize getting on the phone with bankers from the firm to give yourself the best chance of getting an interview. The rest of the process is pretty atypical. From Michigan, they flew out 20-25 candidates to NYC for first round interviews, and then, depending on how you did, you were asked to come back to the office for a 2 interview super day just hours later. Oftentimes the interviews are heavy on behaviorals, with occasional technicals from the guides. They care most about figuring out who you are, as culture is an extremely important part of their firm."
...
jpmorgan_button.onclick = () =>{
    ...
    interviewsum.innerHTML = "JPM has a similar recruiting process to the other top banks. The process starts with a HireVue, and is followed by a 3 interview superday. Superdays are often technical, more so than you would expect out of a bulge-bracket bank. In terms of recruiting as a whole, JPM has moved towards a more centralized model. This means that they are limiting their target schools to just UPenn and some Ivys, leaving Michigan students to network without a traditional campus visit. Without Michigan being a target school for JPM, Michigan students must network well in hopes to land an interview."
...
lazard_button.onclick = () =>{
    ...
    writeup.innerHTML = "Lazard is a top boutique firm. They have a long history of being an extremely intelligent and trustworthy advisor. Lazard can be described as a white shoe, old school firm. They’re known as top tier in financial advisory, while also having an asset management arm. They are also known as being the largest independent bank, working mainly on middle-market deals. While Michigan isn’t a main target school, Lazard has a collegiate work culture, with analyst class representation from Ivys, Duke, Vanderbilt, Notre Dame, and many Big 10 schools."
...
const lincoln_button = document.querySelector('#lincoln')
lincoln_button.onclick = () =>{
    bankname.innerHTML = "Lincoln International"
    writeup.innerHTML = "To be updated"
    interviewsum.innerHTML = "TBD"
    emailformat.innerHTML = "TBD"
    banklogo.src = ""
...
mizuho_button.onclick = () =>{
    ...
    interviewsum.innerHTML = "Mizuho was the first bank to recruit at UMich this past year, coming to campus in mid-March. Networking is not incredibly important, as many candidates receive interviews without more than a call or two, but having a solid network can definitely help land you an interview. When the firm comes to campus, they host a networking event, followed by two interviews with 2 bankers each the following day. These interviews are a mix of technical and behavioral, with a big emphasis on resume questions and current events. If you get past the first round, they have rolling super days throughout the following weeks, consisting of 3 interviews with higher level bankers. These interviews were more behavioral than the first rounds."
...
mk_button.onclick = () =>{
    ...
    interviewsum.innerHTML = "The recruiting process at M. Klein, as one can imagine, is extremely unique. Networking is completely unimportant. They only accept candidates from their 6 target schools (Michigan is included as one). They host coffee chats for one day at each target school, and that is your one opportunity to network unless you have a personal connection to the firm. Based on that conversation, you may be asked to a first round technical interview. If you pass that test, you are invited to a second technical interview the next day, which is known to be extremely difficult: consisting of many technical questions that are off the guide. The final round is then a super day that is typically 4 hours long, with a mix of both behavioral and technical questions."
...
const piper_button = document.querySelector('#pipersandler')
piper_button.onclick = () =>{
...
pjt_button.onclick = () =>{
    ...
    writeup.innerHTML = "PJT Partners is an elite boutique bank, focused on strategic advisory. Although the firm is incredibly young, created in 2015, PJT takes on some of the largest global M&A deals, while maintaining the intimacy of a small boutique firm. PJT is known to have passionate, driven bankers who reflect the firm’s young and entrepreneurial spirit. The firm is generalist throughout both the internship program and full time. PJT has a strong base of Michigan alumni, so they come to campus looking for some U of M sophomores to enter their intern class."
    interviewsum.innerHTML = "It is important to network early and often with PJT. When they come to campus in the spring, they typically have an invite-only networking event, where invites get decided through networking and a resume drop. Getting into that event is pretty important for landing an interview. PJT analysts are also known to be more difficult to reach than other firms, so try to reach out to a variety of analysts. The first round interview typically consists of behaviorals and applied technicals. The super day is typically 4 interviews, two of each type, with technicals known to be more conceptual and M&A focused than the typical firm."
...
rbc_button.onclick = () =>{
    ...
    interviewsum.innerHTML = "It’s important to network with your school’s representatives to get your foot in the door at RBC. Michigan is a target school for the firm, so be sure to attend their campus visit to meet these reps. The first round interview typically sticks to a mixture of experience based questions and basic technicals. The super day is more of the same, consisting of 3 interviews with a mix of behavioral and technicals, with technicals being noted as less intense than the typical firm. At Michigan specifically, the process can be drawn out over many weeks, making it important to continue networking with RBC bankers in between interviews."
...
const stifel_button = document.querySelector('#stifel')
stifel_button.onclick = () =>{
...
const stout_button = document.querySelector('#stout')
stout_button.onclick = () =>{
...
const tdcowen_button = document.querySelector('#tdcowen')
tdcowen_button.onclick = () =>{
...
ubs_button.onclick = () =>{
    ...
    interviewsum.innerHTML = "Michigan is not a target school for UBS, so they typically don’t come to campus during the recruiting process. In order to get an interview at the firm, you definitely should try to network well. UBS typically starts their recruiting process with a HireVue due to the large number of applicants they receive. Then, some candidates get through to a 3 interview super day which consists of a mix of behaviorals and technicals. The technicals are noted to be simple questions from the guides."
    emailformat.innerHTML = "first.last@ubs.com (ie. john.smith@ubs.com)"
    banklogo.src = "images/banklogos/ubs.png"
}

const wellsfargo_button = document.querySelector('#wellsfargo')
wellsfargo_button.onclick = () =>{
