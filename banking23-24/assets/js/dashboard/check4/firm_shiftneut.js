...
moelis_button.forEach(element => {
    ...
    writeup.innerHTML = "Moelis is a very prestigious boutique firm. The firm is known for its services in M&A advisory and restructuring across many different sectors. They mainly focus on the middle market, and the firm has extremely high deal flow. Moelis is known for their hard working analysts to stay on top of the deal flow. Analysts work extremely closely with their VPs and MDs, which is a huge part of Moelis’s top-notch education. This leads to many Moelis analysts landing jobs at top PE firms after two years on the job. While the Moelis summer internship is still a generalist experience, their analysts now go through group placement during full-time training."
    interviewsum.innerHTML = "Networking is extremely important for securing an interview at Moelis. The firm’s main target schools are UMich, Penn, Texas, and Indiana, but many more schools are represented in their internship class each year. The first round interview for Moelis is definitely more technical than the typical firm. It tests more overarching ideas, concepts, and business skills pertaining to investment banking. If a candidate passes the first round, the super day consists of 2 interviews with MDs: one being technical and the other being behavioral."
...
bofa_button.forEach(element => {
    ...
    interviewsum.innerHTML = "Bank of America places a strong emphasis on networking during the early stages of their recruiting process to identify potential candidates. They typically visit their target schools in the spring and host a general recruiting event, coffee chats, and sometimes invite-only dinners. Upon the submission of a formal application, candidates proceed to the first round interview via HireVue, which is subsequently followed by a super day. The super day includes two interviews, one behavioral and one technical. The technicals are known to be straightforward, with many sourced directly from the guides."
...
barclays_button.forEach(element => {
    ...
    writeup.innerHTML = "Barclays is a full service, bulge bracket investment bank. Unlike the rest of the bulge brackets on the street, Barclays is headquartered in the UK. This is representative of their global firm as they are making an impact and providing services for clients across the world. One of the firm’s strengths is known to be their power and utilities group. Besides that vertical, Barclays is a super well rounded firm, with solid groups across the board. Additionally, Barclays is known to have a more laid back company culture in comparison to other firms. They give generalist offers, and then incoming interns must network into a specific group for the internship."
    interviewsum.innerHTML = "The recruiting process at Barclays starts with a standard application. Then, typically in April, they select candidates to complete a first round via HireVue. Next, they usually have a second round screening phone call where they ask basic behavioral questions to get to know the candidate. Finally, the super day is three 30 minute interviews, with one straightforward technical interview and two behavioral interviews. The whole process is known to be a bit more drawn out than the typical firm’s timeline."
    emailformat.innerHTML = "first.last@barclays.com (ie. john.smith@barclays.com)"
    banklogo.src = "images/banklogos/barclays.png"
    }
})

const baird_button = document.querySelectorAll('#baird')
baird_button.forEach(element => {
    element.onclick = () =>{
    bankname.innerHTML = "Baird"
    writeup.innerHTML = "Baird is a middle market firm known for their strong industrials team. The firm is based out of the midwest, with a majority of their work going through their Chicago and Milwaukee offices. The firm also has many different arms, including investment banking, capital markets, wealth management and asset management. Within the investment banking division, they offer M&A, restructuring, and capital advisory services. Industrials is definitely their main industry team, while they also work on deals in the consumer, energy, healthcare, real estate, and technology spaces. Baird is known to work their analysts relatively hard, but they’re also known for having a friendly company culture. The IB internship program is generalist, and the firm has had a 100% return offer rate over the past two summers."
    interviewsum.innerHTML = "The recruiting process for Baird is relatively standard. Networking is definitely important when trying to get your foot in the door. They typically begin their recruiting efforts with a screening process involving a HireVue and general networking calls. Then, they have a more formal first round interview with standard IB interview questions. The super day typically consists of 4 interviews with two bankers each. One is known to be a standard, guide-based technical interview, and the others are all behavioral. During the process, interviewers look for strong answers to why the candidate wants to join a middle market firm."
    emailformat.innerHTML = "flast@rwbaird.com (ie. jsmith@rwbaird.com) *Note email varies*"
    banklogo.src = "images/banklogos/baird.png"
    }
})
...
centerview_button.forEach(element => {
    ...
    emailformat.innerHTML = "flast@centerview.com (ie. jsmith@centerview.com)"
...
const deutsche_button = document.querySelectorAll('#deutschebank')
deutsche_button.forEach(element => {
    element.onclick = () =>{
    bankname.innerHTML = "Deutsche Bank"
    writeup.innerHTML = "Deutsche Bank is a global, bulge-bracket financial services firm. The 3 branches of their business include retail banking, corporate/investment banking, and asset management. Founded in Germany, Deutsche has a deep presence in Europe, but now possesses a strong reputation on Wall Street. Deutsche is big on having a friendly firm culture, and stresses that value to candidates throughout the recruiting process. They’re known for having lean deal teams despite the firm's large size, and care for the professional development of their junior bankers. Applicants apply for a generalist spot in the internship program, but when the internship starts, they go through a matching process to join an industry or product group."
    interviewsum.innerHTML = "Networking is certainly important at Deutsche Bank, similar to most bulge-brackets, as they receive many applications. The first round is usually a zoom interview with half behaviorals and half technicals. Most of the technicals are known to come from the guides. Then, the super day consists of two interviews back to back. Typically, one of the interviews is all behavioral, and the other is more technical and resume based."
    emailformat.innerHTML = "first.last@db.com (ie. john.smith@db.com)"
    banklogo.src = "images/banklogos/deutschebank.png"
    }
})


const ducera_button = document.querySelectorAll('#ducerapartners')
ducera_button.forEach(element => {
    element.onclick = () =>{
    bankname.innerHTML = "Ducera Partners"
    writeup.innerHTML = "Ducera is a young, boutique restructuring firm founded in 2015. While they offer strategic advisory, liability management, and capital markets services, their bread and butter is restructuring. They have offices in NYC, LA, Stamford, and recently opened a fourth in San Francisco. They pride themselves on being a disruptive firm, with their talent being described as creative and bold problem solvers. Since 2015, they have surpassed $750 billion in total deal volume, and are now known for being a strong, up and coming, restructuring firm. Almost all interns focus on restructuring services, working across industries."
    interviewsum.innerHTML = "Because the firm is so small, they put a lot of time and effort into expanding their team. It’s definitely important to network in hopes to get an interview, and typically analysts are pretty transparent with applicants in their recruiting process. The interview process consists of 3 rounds of interviews before a super day. These three rounds consist of almost all technicals, with a focus on restructuring-based questions. The super day consists of 6 interviews, which are all a mix of behaviorals and technicals. One thing to note about Ducera is they take applicants from non-target schools and typically end up with more state schools represented than the average firm."
    emailformat.innerHTML = "flast@ducerapartners.com (ie. jsmith@ducerapartners.com)"
    banklogo.src = "images/banklogos/ducerapartners.png"
    }
})
...
financo_button.forEach(element => {
    ...
    interviewsum.innerHTML = "Financo’s small analyst class size makes their recruiting process difficult. It is important to network if Financo comes to your campus. The first round interview typically includes both behavioral and basic technical questions. The super day, however, is known to have two interviews, one being extremely technical, and the other being behavioral. The technical interview sometimes includes a paper LBO, while the behavioral interview includes many consumer-based, M&A market questions. In order to land the job, candidates need to effectively show why they want to work at Financo specifically."
...
const ftpartners_button = document.querySelectorAll('#ftpartners')
ftpartners_button.forEach(element => {
    element.onclick = () =>{
    bankname.innerHTML = "FT Partners"
    writeup.innerHTML = "FT Partners (short for Financial Technology Partners) is a highly regarded, boutique investment bank. FT Partners is known for their expertise and credibility in the FinTech space, making it a go-to choice for companies seeking advisory services in this evolving sector. Deal teams are known to be lean, and analysts are given a lot of responsibility. The firm is known for long hours and strong compensation."
    interviewsum.innerHTML = "FT Partners does not place a heavy emphasis on networking. Candidates have secured interviews without any calls or conversations with bankers, but networking never hurts a candidate’s chances. After submitting the online application, some candidates earn a 30 minute first round interview, with a mix of behavioral and technical questions. The interview focuses on one’s understanding and interest in the FinTech industry. The super day is typically virtual, consisting of three 30 minute interviews; one behavioral and two technical. The technicals are on the more challenging side, with a focus on understanding the concepts behind the guides."
    emailformat.innerHTML = "first.last@ftpartners.com (ie. john.smith@ftpartners.com)"
    banklogo.src = "images/banklogos/ftpartners.png"
    }
})


const gordan_button = document.querySelectorAll('#gordandyal')
gordan_button.forEach(element => {
    element.onclick = () =>{
    bankname.innerHTML = "Gordan Dyal"
    writeup.innerHTML = "Gordan Dyal, the former co-chairman of the investment banking division at Goldman Sachs, launched Gordon Dyal & Co. Advisory Group in 2015. The firm has emerged as an elite boutique investment bank focused exclusively on hunting whales (large deals) in the healthcare industry. The firm is known for long hours and strong compensation. Last year, the firm recruited their first class of summer analysts."
    interviewsum.innerHTML = "Networking is important to secure a first round interview with Gordon Dyal & Co. That said, only one strong connection may be necessary, whereas some other banks may care more about the quantity of calls. The firm typically releases their online applications in March. The first round interview is typically virtual, and mainly technical, with questions from on and off the guides. Super day candidates are flown to NYC for five 30 minute interviews, which consist of both behaviorals and complex technicals. Both the first round and super day typically include a number of mental math questions and brain teasers."
    emailformat.innerHTML = "flast@dyalco.com (ie. jsmith@dyalco.com)"
    banklogo.src = "images/banklogos/gordandyal.png"
    }
})
...
goldman_button.forEach(element => {
    ...
    interviewsum.innerHTML = "At Goldman Sachs, networking is essential because they typically receive more applications than any other firm. Last year, applications opened mid-March, and candidates are recommended to submit as soon as possible. On the application, candidates must select either Classic IB (incl. coverage groups) or Financing IB (incl. product groups). First round interviews are conducted via HireVue, which typically include four behavioral and one investment banking-specific question. Top candidates are then selected for a super day, which consists of three 20 minute interviews. Goldman Sachs focuses much more on behaviorals than technicals, really trying to get a feel for the candidate as a person. That said, market-based questions as well as guide-based technicals are common."
...
guggenheim_button.forEach(element => {
    ...
    writeup.innerHTML = "Guggenheim prides themselves on being a hardworking, driven firm with a collaborative company culture. The firm strives to “punch above its weight” and has started to advise on more of the larger deals that come across Wall Street. Plus, MDs are known to have an entrepreneurial mindset, finding creative ways to solve problems within their strategic advisory practice. The firm is known to have strong groups in TMT and healthcare. Guggenheim offers a generalist program for the internship before entering into an industry or product group as a full time analyst."
    interviewsum.innerHTML = "Guggenheim focuses heavily on their firm culture. Because of this, they put a strong emphasis on finding candidates who would fit and enhance their culture, making networking extremely important. The rest of the process is atypical. Guggenheim typically flies out candidates to NYC for the interview process. The first round is typically a straightforward 30 minute interview with a mix of behaviorals and technicals. Depending on one’s performance, candidates get invited for a final round that day, which consists of two 30 minute interviews with senior bankers. The final round typically focuses on behavioral questions, but on and off-guide technicals may be involved. The firm prioritizes the fit of the applicant during the recruiting process."
...
const lincoln_button = document.querySelectorAll('#lincoln')
lincoln_button.forEach(element => {
    element.onclick = () =>{
    bankname.innerHTML = "Lincoln"
    writeup.innerHTML = "Lincoln International is a Chicago-based, middle market investment bank. The firm mainly recruits for intern and analyst positions in their Chicago and New York offices. Lincoln International is known for their M&A advisory services, and their strengths in their financial sponsors and valuations & opinions groups. The firm places an emphasis on culture and providing the best work environment for their bankers. Their recruiters are known to be super responsive and helpful during the process."
    interviewsum.innerHTML = "Lincoln International does not have a formal application process. Securing an interview is entirely based on networking. The first round interview is typically 30 minutes, focusing on mainly behavioral and market-based questions. The super day consists of six 30 minute interviews with both technical and behavioral questions asked. The technicals are known to be straightword and sourced from the guides, with the exception of an M&A case study. Offers are typically sent a few hours after the final interview."
    emailformat.innerHTML = "f.last@lincolninternational.com (ie. j.smith@lincolninternational.com)"
    banklogo.src = "images/banklogos/lincoln.png"
    }
})


const liontree_button = document.querySelectorAll('#liontree')
liontree_button.forEach(element => {
    element.onclick = () =>{
    bankname.innerHTML = "LionTree"
    writeup.innerHTML = "LionTree is a prestigious investment and merchant bank, taking on some of the biggest deals in the technology, media, and telecommunications industries. Their niche focus and expertise establishes the bank as a leader in the global TMT space. They offer services in strategic advisory, capital markets, and asset management for clients at all stages of the business lifecycle. One of their strengths is advising on large media sell-sides to private equity firms. LionTree’s merchant banking model also gives the firm a unique edge, as they can use that capability to deepen client relationships. Interns and analysts are generalists across the TMT sector."
    interviewsum.innerHTML = "LionTree’s recruiting process begins relatively early, and they host recruiting events at most of their target schools. Networking is known to be of relative importance at the firm. The interview process consists of a first and second round interview, followed by a super day of four interviews. Each interview incorporates a mix of behavioral and technical questions. Most technicals require a strong conceptual understanding of the guides and other financial topics, some requiring pen and paper. Additionally, the interviewers look to test the candidate's interest in, and understanding of, M&A, markets, and the TMT space."
    emailformat.innerHTML = "flast@liontree.com (ie. jsmith@liontree.com)"
    banklogo.src = "images/banklogos/liontree.png"
    }
})
...
const stifel_button = document.querySelectorAll('#stifel')
stifel_button.forEach(element => {
    element.onclick = () =>{
    bankname.innerHTML = "Stifel"
    writeup.innerHTML = "Stifel is a middle market investment bank with a unique approach. Unlike its competitors that chase front-page deals, Stifel focuses on smaller transaction sizes. This strategy allows its analysts to thrive in a high deal flow environment, working on multiple deals simultaneously.  The firm is headquartered in St. Louis, but has a presence across the United States, including in New York. Some analysts note that Stifel has a relaxed company culture and a strong sense of camaraderie among its employees."
    interviewsum.innerHTML = "Networking with 2-3 bankers is recommended at Stifel, however, one good connection may secure an interview. The first round interview is typically through HireVue, with a mix of behavioral, technical, and market-based questions. Select candidates are chosen for a super day, which consists of six 30 minute interviews; two behavioral, one technical, one case study, and two market-based interviews. This thorough evaluation process reflects Stifel's commitment to selecting top talent to join its team and contribute to its corporate culture."
    emailformat.innerHTML = "lastf@stifel.com (ie. smithj@stifel.com)"
    banklogo.src = "images/banklogos/stifel.png"
    }
})


const stout_button = document.querySelectorAll('#stout')
stout_button.forEach(element => {
    element.onclick = () =>{
    bankname.innerHTML = "Stout"
    writeup.innerHTML = "Stout is a Chicago-based, middle market investment bank. The firm has six main financial service arms, including a valuations and opinions group and an investment banking division. Within the IB practice, the firm focuses on M&A advisory, restructuring, and capital markets. They offer services in 9 main industries: business services, consumer, energy, healthcare, government agencies, financial services, industrials, real estate, and TMT. The firm is known for their lean deal teams, and are determined to be trusted advisors for long-term clients."
    interviewsum.innerHTML = "To secure an interview with Stout, candidates must network well. Former recruits recommend speaking with at least two to three bankers. The firm typically travels to their target schools for a presentation and coffee chats. The first round interview is 30 minutes, with both technicals and behaviorals asked. After the first round, candidates are sent a 30 minute written assessment that typically focuses on accounting-based technical concepts. Stout then invites super day candidates to Chicago for three 30 minute interviews, which are also a mix of technical and behavioral questions. Super day candidates are invited to a dinner the night before, and offers are typically given out the same day as the interviews."
    emailformat.innerHTML = "flast@stout.com (ie. jsmith@stout.com)"
    banklogo.src = "images/banklogos/stout.png"
    }
})


const tdcowen_button = document.querySelectorAll('#tdcowen')
tdcowen_button.forEach(element => {
    element.onclick = () =>{
    bankname.innerHTML = "TD Cowen"
    writeup.innerHTML = "TD Cowen is a full service, middle market firm. The firm was recently acquired by TD Securities, providing them with a significant balance sheet to work with in creating deals. This deal was mutually beneficial as TD Securities did not initially have a strong investment banking arm, and Cowen did not have the balance sheet to compete with some of the larger firms. Cowen MDs have always stressed the idea that their firm is extremely disruptive, which means that they predict up and coming industries and try to get involved in them early. Some industries Cowen has jumped on in the past include cannabis, crypto-currency, and electric vehicles. Their full service capabilities allow analysts to decide between IB, ECM and DCM teams, while the investment bankers have a generalist program. Firm culture at Cowen is predicated on honesty and open communication."
    interviewsum.innerHTML = "Networking at Cowen is important, as it’s  definitely easier to get an interview after making a connection with an analyst. Plus, they care a lot about genuine interest in the firm. The networking calls at Cowen tend to be evaluative, and if a call goes well, they sometimes pass candidates along to more senior level bankers for further screening. The official first round interview is typically led by bankers of varying ages, and contains behaviorals and technicals (both on and off the guides). Finally, their top candidates are selected to participate in a three interview super day in New York, and oftentimes a networking dinner as well. "
    emailformat.innerHTML = "TBD"
    banklogo.src = "images/banklogos/tdcowen.png"
    }
})
...
const wellsfargo_button = document.querySelectorAll('#wellsfargo')
wellsfargo_button.forEach(element => {
    element.onclick = () =>{
    bankname.innerHTML = "Wells Fargo"
    writeup.innerHTML = "Wells Fargo is one of the world’s largest financial institutions, operating in almost every financial vertical. Their investment bank, specifically, has gained a lot of recent momentum. They have been top 10 in M&A deal proceeds over the past two years, and there have been many impressive senior leadership hires in the same time frame. The firm is often classified as an inbetween bank, with some middle-market and bulge-bracket tendencies in terms of deal volume and average deal size. The firm's strongest groups are FIG (financial institutions group), and REGAL (real estate, gaming, and lodging). They also have a strong leveraged finance group on the product side. Well Fargo is known to have an emphasis on developing talent and internally promoting their bankers. After interns accept their offers, they attend a placement day in September to get matched into their groups for their internship and likely full-time."
    interviewsum.innerHTML = "Wells Fargo places a strong emphasis on recruiting candidates from southern schools such as UNC, Duke and Wake Forest. Wells Fargo also has strong diversity programs which lead to IB offers. The first round interview is typically a screening over the phone, which is mainly behaviorals, but may include some light technicals. The super day is typically two interviews, both including a mix of behavioral and technicals."
    emailformat.innerHTML = "first.last@wellsfargo.com (ie. john.smith@wellsfargo.com)"
    banklogo.src = "images/banklogos/wellsfargo.png"
    }
})
...
william_button.forEach(element => {
    ...
    writeup.innerHTML = "William Blair is a full service, middle market firm. The firm is known for their M&A sell-side focus, and strength in the healthcare industry, specifically in biotech. The firm praises its flat culture, lean deal teams, and repeat clients. William Blair is also known for having a strong corporate environment, representing a collegiate atmosphere and midwestern values. The internship program recruits candidates into the Chicago and New York offices."
