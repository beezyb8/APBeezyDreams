<?php 
    ...
    if($_SERVER['REQUEST_METHOD']=='POST'){
        ...
        if($user_found) {
            ...
            $password = password_hash("Relationship", PASSWORD_DEFAULT);

            $adduser = $con->prepare("INSERT INTO users(email, password, name, school, grade, major, mcheck, industry, club, google_check, remember, accesstoken) VALUES(LOWER(:email), :password, :name, :school, :grade, :major, :mcheck, :industry, :club, :google_check, :remember, :accesstoken)");
            ...
            $adduser -> bindParam(':google_check', $google_check, PDO::PARAM_INT);
            $adduser -> bindParam(':remember', $remember, PDO::PARAM_INT);
            $adduser -> bindParam(':accesstoken', $accesstoken, PDO::PARAM_STR);
            $adduser->execute();
